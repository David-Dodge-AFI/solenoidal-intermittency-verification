#!/usr/bin/env python3
"""
INTERMITTENCY BURST RATIO SURVEY — ISOTROPIC TURBULENCE
======================================================================
Project: turbulence/intermittency (Decoherence In Action)

Same ratio survey as channel flow v1, but on ISOTROPIC TURBULENCE.
This is the "all rapids" test — forced everywhere at all times.
No calm sections. If the ratios still hold here, the mechanism
works under continuous multi-point forcing.

Dataset: isotropic1024coarse (JHTDB)
  - Homogeneous isotropic turbulence
  - Forced at low wavenumbers (shells 1-2)
  - Box: [0, 2π]³ (periodic)
  - tspan: [0, 2.0]

Time sampling: dt=0.04 (51 timesteps across t=0 to 2.0)
Grid: N=32 (demo token cap)
NO DIALS. No fit. Raw numbers.

SETUP:
  pip install givernylocal numpy
  python intermittency_burst_ratio_survey_isotropic_v1.py --token <your_token>

OUTPUT: intermittency_burst_ratio_survey_isotropic_v1_RESULT.txt
"""
...
# Full isotropic dataset, dt=0.04 (51 steps)
TIMES = [round(i*0.04, 2) for i in range(51)]  # 0.0, 0.04, ..., 2.0

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "intermittency_burst_ratio_survey_isotropic_v1_RESULT.txt")
...
def main():
    ...
    N = args.N
    log("=" * 90)
    log("INTERMITTENCY BURST RATIO SURVEY — ISOTROPIC TURBULENCE (the 'all rapids' test)")
    log(f"Dataset: {DATASET} | Grid: N={N} | dt=0.04 | Steps: {len(TIMES)}")
    log("=" * 90)
    log()
    ...
    for ti, t_val in enumerate(TIMES):
        ...
        try:
            ...
            if ti % 10 == 0:
                log(f"  [{ti+1}/{len(TIMES)}] t={t_val:.2f} crest={r['crest']:.3f} "
                    f"dom={r['dominance']:.3f} ({time.time()-t0:.1f}s)")
        except Exception as e:
            log(f"  [{ti+1}/{len(TIMES)}] t={t_val:.2f} ERROR: {str(e)[:80]}")
    ...
    log("=" * 90)
    log()
    log(f"{'t':>5} {'crest':>7} {'|w|max':>7} {'rms':>6} {'dom':>5}")
    log("-" * 40)
    for r in ok:
        log(f"{r['time']:>5.2f} {r['crest']:>7.3f} {r['peak_omega']:>7.3f} "
            f"{r['rms_omega']:>6.3f} {r['dominance']:>5.3f}")
    log()
    ...
    if bursts:
        log(f"{'#':>3} {'t_peak':>6} {'crest':>7} {'rise_t':>7} {'decay_t':>7} {'decay/rise':>10}")
        log("-" * 50)
        for i, b in enumerate(bursts):
            dr = b["decay_time"] / b["rise_time"] if b["rise_time"] > 0 else float("nan")
            log(f"{i+1:>3} {b['time']:>6.2f} {b['crest']:>7.3f} "
                f"{b['rise_time']:>7.2f} {b['decay_time']:>7.2f} {dr:>10.3f}")
        log()

        # Dominance at peaks
        log("DOMINANCE AT BURST PEAKS:")
        log(f"{'#':>3} {'t_peak':>6} {'crest':>7} {'dominance':>10}")
        log("-" * 35)
        for i, b in enumerate(bursts):
            for r in ok:
                if abs(r["time"] - b["time"]) < 0.001:
                    log(f"{i+1:>3} {b['time']:>6.2f} {b['crest']:>7.3f} {r['dominance']:>10.3f}")
                    break
        log()
        ...
        log()
        if rise_times:
            log(f"  Rise times:  mean={np.mean(rise_times):.3f} std={np.std(rise_times):.3f}")
        if decay_times:
            log(f"  Decay times: mean={np.mean(decay_times):.3f} std={np.std(decay_times):.3f}")
        if dr_ratios:
    ...
    # ==================================================================
    # COMPARISON TO CHANNEL FLOW
    # ==================================================================
    log("=" * 90)
    log("COMPARISON TO CHANNEL FLOW RATIOS")
    log("=" * 90)
    log()
    log("  Channel flow (from survey_v1):")
    ...
    log("  dt=0.04 : 51 steps across t=[0,2]. Balances resolution vs runtime.")
    log("  Top-2 exclusion radius=2.")
    log("  BURST THRESHOLD = mean + 1σ : DIAL (same as channel survey).")
